...
/*!
 * \file
 * \copydoc Opm::DecaneCO2ParameterCache
 */
#ifndef OPM_DECANECO2_PARAMETER_CACHE_HPP
#define OPM_DECANECO2_PARAMETER_CACHE_HPP
...
class DecaneCO2ParameterCache
    : public Opm::ParameterCacheBase<DecaneCO2ParameterCache<Scalar, FluidSystem> >
{
    typedef DecaneCO2ParameterCache<Scalar, FluidSystem> ThisType;
...
public:
    ...
    typedef Opm::PengRobinsonParamsMixture<Scalar, FluidSystem, oilPhaseIdx, /*useDecaneCo2=*/true> OilPhaseParams;
    //! The cached parameters for the gas phase
    typedef Opm::PengRobinsonParamsMixture<Scalar, FluidSystem, gasPhaseIdx, /*useDecaneCo2=*/true> GasPhaseParams;

    DecaneCO2ParameterCache()
    {
...
protected:
    ...
    {
        Scalar T = Opm::decay<Scalar>(fluidState.temperature(phaseIdx));
        Scalar p = Opm::decay<Scalar>(fluidState.pressure(phaseIdx));
                            ...
                            unsigned phaseIdx)
    {
        ...
        case gasPhaseIdx: {
            ...
            // here...
            Vm_[gasPhaseIdx] = Opm::decay<Scalar>(
        ...
        case oilPhaseIdx: {
            ...
            // system can get queried, so it is okay to calculate it
            // here...
            Vm_[oilPhaseIdx] = Opm::decay<Scalar>(
                PengRobinson::computeMolarVolume(fluidState,
                                                 *this,
                                                 phaseIdx,
                                                 /*isGasPhase=*/false));
        ...
        case waterPhaseIdx: {
            ...
            Scalar overPressure = Opm::decay<Scalar>(fluidState.pressure(waterPhaseIdx) - 1.013e5); // [Pa]
            Scalar waterDensity =
                stockTankWaterDensity * (1 + 3.3e-6*overPressure/6894.7573);

            // convert water density [kg/m^3] to molar volume [m^3/mol]
            Vm_[waterPhaseIdx] = Opm::decay<Scalar>(
                                 fluidState.averageMolarMass(waterPhaseIdx)/waterDensity);
            break;
        };
        };
    }
